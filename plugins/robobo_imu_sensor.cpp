...
void gazebo::RoboboImuSensor::Load(gazebo::sensors::SensorPtr sensor_, sdf::ElementPtr sdf_)
{
  ...
  accel_data_publisher = node->advertise<geometry_msgs::Accel>(robot_namespace + "accel",1);
  orient_data_publisher = node->advertise<geometry_msgs::Quaternion>(robot_namespace + "orientation",1);

  connection = gazebo::event::Events::ConnectWorldUpdateBegin(boost::bind(&RoboboImuSensor::UpdateChild, this, _1));
